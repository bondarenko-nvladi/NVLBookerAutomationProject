...
public class UpdatedBookingTest {
    ...
    public void testUpdateBooking() throws Exception {
        ...
        //создаем объект букинг с необходимыми данными по обновлению
        NewBooking updatedBooking = new NewBooking();
        updatedBooking.setFirstname("Stanly");
        updatedBooking.setLastname("Doe");
        updatedBooking.setTotalprice(170);
        updatedBooking.setDepositpaid(true);
        updatedBooking.setBookingdates(new BookingDates("2024-01-01", "2024-01-05"));
        updatedBooking.setAdditionalneeds("Breakfast");

        //Отправляем PUT запрос
        Response updateResponse = apiClient.updateBooking(bookingId, objectMapper.writeValueAsString(updatedBooking));
        ...
        NewBooking result = objectMapper.readValue(updateResponseBody, NewBooking.class);

        // 5. Проверяем обновленные поля
        assertThat(result.getFirstname()).isEqualTo("Stanly");
        assertThat(result.getTotalprice()).isEqualTo(170);
